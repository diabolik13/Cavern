...
a = 8.1e-27  # creep material constant, [Pa^n]
bo = 1e-6
...
imp1 = mesh.peak(4000, -300, -4000) / 3
imp2 = mesh.peak(100, -300, -200) / 12
imp3 = mesh.onedpeak(20, 100) / 4

imp_tot = imp1 + imp2 - imp3
imp_tot = imp_tot / abs(np.max(imp_tot)) / 5
# plot_parameter(mesh, imp_tot)

imp = np.zeros((mesh.nele(),))
for elt in range(mesh.nele()):
    nodes = mesh.cells(elt)
    imp[elt] = np.mean(imp_tot[nodes])
...
if NR == False:
    if nt > 1:
        for i in tqdm(range(nt - 1)):
            ...
            sw_eq = 1 / 2 * (s_I + svmg)
            omega = b * sw_eq ** kk / (1 - omega) ** l * dt
            if np.max(omega) > 0.9:
                omega[omega > 0.9] = 0.9

            f_cr, strain_crg = V.creep_load_vector(dt, a, n, q, r, temp, stressg, strain_crg, arrhenius, omega=None)
